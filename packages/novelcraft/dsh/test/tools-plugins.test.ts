...
//      服务缺失时构造抛错(fail-closed, 不静默)。
//   ③ 单独挂载与默认路径共享 build 函数: 注册的工具名集合与按开关过滤的默认路径一致。
...
describe('工具组独立插件(inject novelcraft; 组合式 profile)', () => {
  it('NovelcraftMapAtlasPlugin 单独挂载 → 只注册 6 个地图册工具', async () => {
    const env = await setup({ tools: { mapAtlas: false } });
    expect(env.tools.length).toBe(15);
  ...
  it('NovelcraftWritingToolsPlugin 单独挂载 → 15 个写作/存储工具(与开关过滤同集合)', async () => {
    const env = await setup({ tools: { writing: false } });
    await env.h.ctx.plugin(NovelcraftWritingToolsPlugin);
    const writing = env.tools.filter((t) => !t.name.startsWith('novelcraft_map_atlas_'));
    expect(writing.length).toBe(15);
  });
