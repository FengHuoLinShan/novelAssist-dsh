...
import { signalLogicalKey, type CreateSignalInput } from "./signals.js";

export interface SuggestRadarObject {
  slug: string;
  name: string;
  entity_type: string;
  status: string;
...
export function scanSuggestRadar(root: string, now?: Date): RadarReconcileResult {
  return reconcileRadarSignals(root, "suggest-", collectSuggestRadarHits(root), now);
}

export function collectSuggestRadarHits(root: string, objects: SuggestRadarObject[] = listObjects(root)): CreateSignalInput[] {
