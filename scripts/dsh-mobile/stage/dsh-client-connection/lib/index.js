...
//#endregion
//#region lib/types/web-pairing.js
/**
* Device-pairing gate for LAN/Tailscale/tunnel deployments. The Host fence
* ([api-request-trust](./api-request-trust.ts)) is a DNS-rebinding defense,
* explicitly not authentication: once a deployment binds beyond loopback (or
* trusts an external authority), anyone whose Host passes the fence can drive
* the RPC bridge — including agent sessions that execute tools on the host.
* Pairing closes that gap with a per-process PIN: non-loopback requests must
* carry a token issued by `POST /api/pair`, while loopback requests stay open
* so desktop use never changes. Tokens live in memory only — a server restart
* prints a fresh PIN and requires re-pairing.
*/
/** Pairing endpoint path (exact-route intercept inside the /api prefix). */
const PAIR_PATH = "/api/pair";
/** Session cookie carrying the issued pairing token. */
const PAIR_COOKIE = "dsh-pair";
/** WWW-Authenticate challenge value the browser half reads to surface pairing UI. */
const PAIR_CHALLENGE = "DSH-Pair";
/** Failed PIN attempts tolerated per rolling window before lockout. */
const PAIR_MAX_FAILURES = 5;
/** Rolling failure window in milliseconds. */
const PAIR_WINDOW_MS = 60_000;
/** Pairing cookie lifetime: 30 days, so a paired phone survives browser restarts. */
const PAIR_COOKIE_MAX_AGE = 60 * 60 * 24 * 30;
/** Constant-time PIN comparison: compare SHA-256 digests so lengths never leak timing. */
function timingSafeEqualText(a, b) {
	const digest = (text) => createHash("sha256").update(text, "utf8").digest();
	return timingSafeEqual(digest(a), digest(b));
}
/** The `dsh-pair` token from a Cookie header, if present and non-empty. */
function pairingToken(headers) {
	const cookie = header(headers, "cookie");
	if (cookie === void 0) return;
	for (const part of cookie.split(";")) {
		const at = part.indexOf("=");
		if (at === -1) continue;
		if (part.slice(0, at).trim() !== PAIR_COOKIE) continue;
...
function apply(ctx, config) {
	...
	const pairPin = config?.pairPin ?? "";
	const pairingEnabled = pairPin !== "";
	const pairedTokens = /* @__PURE__ */ new Set();
	const pairFailures = [];
	/** Host authority hostname of one request (WHATWG-normalized). */
	const requestHostname = (req) => {
		const host = header(req.headers, "host");
		return host === void 0 ? void 0 : parseAuthority(host)?.hostname;
	};
	/** Loopback requests bypass pairing; everything else must carry an issued token. */
	const isPaired = (req) => {
		if (!pairingEnabled) return true;
		const hostname = requestHostname(req);
		if (hostname === void 0) return false;
		if (isLoopbackHostname(hostname)) return true;
		const token = pairingToken(req.headers);
		return token !== void 0 && pairedTokens.has(token);
	};
	/** `GET` probes pairing state; `POST` verifies a PIN and issues the token cookie. */
	const handlePair = async (req, res) => {
		const hostname = requestHostname(req);
		const loopback = hostname !== void 0 && isLoopbackHostname(hostname);
		const ok = (payload, status = 200, headers = {}) => {
			res.writeHead(status, { "content-type": "application/json; charset=utf-8", ...headers });
			res.end(JSON.stringify(payload));
		};
		if (req.method === "GET") {
			ok({ paired: loopback || !pairingEnabled || isPaired(req) });
			return;
		}
		if (req.method !== "POST") {
		...
		if (!pairingEnabled || loopback) {
			ok({ paired: true });
			return;
		}
		const now = Date.now();
		while (pairFailures.length > 0 && now - pairFailures[0] > PAIR_WINDOW_MS) pairFailures.shift();
		if (pairFailures.length >= PAIR_MAX_FAILURES) {
			ok({ error: "rate-limited" }, 429, { "retry-after": "60" });
			return;
		}
		let pin;
		try {
			...
			for await (const chunk of req) {
				...
				if (received > 4096) throw new Error("body too large");
				chunks.push(chunk);
			}
			const parsed = JSON.parse(Buffer.concat(chunks).toString("utf8"));
			pin = typeof parsed === "object" && parsed !== null ? parsed.pin : void 0;
		} catch {
			pin = void 0;
		}
		if (typeof pin !== "string" || !timingSafeEqualText(pin, pairPin)) {
			pairFailures.push(now);
			ok({ error: "invalid-pin" }, 401, { "www-authenticate": PAIR_CHALLENGE });
			return;
		}
		pairFailures.length = 0;
		const token = randomBytes(32).toString("hex");
		pairedTokens.add(token);
		ok({ paired: true }, 200, { "set-cookie": `${PAIR_COOKIE}=${token}; HttpOnly; SameSite=Strict; Path=/api; Max-Age=${PAIR_COOKIE_MAX_AGE}` });
		ctx.logger?.info(`client-connection: paired non-loopback client ${hostname ?? "(unknown)"}`);
	};
	...
	const route = {
		...
		handler: async (req, res) => {
			...
			if (!isPaired(req)) {
				res.writeHead(401, { "www-authenticate": PAIR_CHALLENGE, "content-type": "application/json; charset=utf-8" });
		...
		const registerDownlink = (path, handle) => {
			apiCtx.effect(() => apiCtx.webServer.registerUpgrade({
				...
				handler: (req, socket, head) => {
					...
					if (!isPaired(req)) {
						socket.end(["HTTP/1.1 401 Unauthorized", "Connection: close", "Content-Type: text/plain; charset=utf-8", "Content-Length: 16", "", "pairing-required"].join("\r\n"));
						return;
					}
