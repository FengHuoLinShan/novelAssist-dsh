...
* @param pairPin - invocation PIN non-loopback clients must pair with ("" = pairing off).
 * @returns the LAN display addresses and invocation-derived fence authorities.
*/
function resolveLanTrust(bindHost, extra, pairPin = "") {
...
function apply(ctx, config) {
	const runtime = resolveLanTrust(ctx.webServer.host, config.trustedHosts, config.pairPin);
	...
	if (config.printUrl) {
		const printUrl = () => {
			...
			const lan = runtime.lanAddresses.map((addr) => `http://${addr}:${String(port)}`).join(", ");
			console.log(`dsh web: ${localWebUrl(ctx)}${lan === "" ? "" : ` (LAN: ${lan})`}`);
			if (runtime.pairPin !== "") console.log(`dsh web: pairing PIN for non-local clients: ${runtime.pairPin}`);
		};
