...
| N47 | RAG current-source typed result(total/truncated/open_target) | ①core rag 加法: RagSearchResult 增 total(召回截断前数量)与 truncated(恒布尔, topK 截断标记)——调用方可信判断「8 条结果」≠「只有 8 条」(台账 §6.12.6「无 total/truncated」缺口); RagChunk 增 open_target(确定性派生不改落盘: 章正文 → chapters/NNN.md 相对路径 + start/end 偏移; outline → structure/; memory → memory/events.jsonl; 其余 → world/)——「来源重开」的最小读面。②dsh novelcraft_rag_search 回执: total/truncated 扁平字段 + hits 附 open_path/open_start/open_end; message 含 命中 n/total 与截断提示。依据: 后续开发计划.md M12 + 台账 §6.12.6 + 铁律 4(纯加法, 落盘格式不变)。 |
