...
| N44 | outline 生成 preview/apply 拆分与生成域工具化 | M12-b 落地: ①core outline 加法 `preview.ts`: previewStoryOutline/previewOutlineItem(跑 spec → 结果+promptFingerprint 暂存 .assistant/proposals/, 照 writing.proposeNextChapter 先例, **不写 structure 资产**)+applyStoryOutlinePreview/applyOutlineItemPreview(读暂存校验 kind/run_id/result 形态 → writeOutline/writeStructureAsset); 生成输出→frontmatter 的确定性映射抽为 outlineItemFrontmatter 导出(2.2 与 apply 共用同一语义, 防漂移)。②dsh `outline-face.ts` + 9 工具(归 writing 组): outline_preview/outline_item_preview(propose, 暂存零 canonical 写)、outline_apply/outline_item_apply(adoptGuarded, 审批内 canonical 写)、world_chat/converge/explore/inspect(propose, 纯 LLM 零写)、world_bible_suggest(propose, 落 bible/ draft 提案——采用另走 store_adopt, §6.17 页面建议只落工作稿语义)。③store **object** schema 的 kind 加 enums: ENTITY_TYPES(20 类; pending 刻意不加 —— pending 是导入原始候选的宽松落点, canonical 采纳门才收紧, 两层设计而非遗漏)。④world slug 坍缩错误语义区分(同名 vs 坍缩冲突)。M12-b review 修复(2026-09-01, 无 P0, 2×P1+7×P2 全闭): ①P1 slug「坍缩区分」原实现恒真死代码(比对 slug 与其自身定义) → 改为比对既有对象的 name(同名 vs 坍缩可分辨, 测试锁定同名分支); ②P1 kind enums 打断 deep-import→adopt 链(LLM 原始 entity_type 直写 pending, adopt 时 INVALID_ENUM 永拒)→ imports entities.ts 落盘前 normalizeEntityType 归一(白名单外回落 object, 原始值进 note 注记不丢失)+ INVALID_ENUM 负例锁定; ③P2: 工具层 KINDS 回收为 store ENTITY_TYPES 同源(20 类)/preview run_id 加 6 hex 随机熵(同毫秒不覆盖)+ readPreview 匹配带 target 消歧/apply 总纲白名单透传(被编辑暂存不得注入未知 frontmatter 键)/signal 全链透传(工具→service→face→provider, 内层 1800s 可被外层取消)/result_json+reply 走 receiptLimit 截断(N39② 延伸)/N44③ 文本修正(pending 无 enums 是两层设计); 测试补: bible_suggest 行为(bible/ draft+status=draft+精确 git 提交)/暂存篡改负例/kind enums 负例/slug 同名分支。已知边界(记 M12-c): outline_item preview 的 target 匹配仍靠文件名约定(proposalFile slug 化对中文 target 的稳定性)、input_hash 存而不校验(apply 不重算输入一致性)、world 生成中心「已存在」冲突的裸 Error 契约不一致。依据: 后续开发计划.md M12-b + 台账 §6.17/§6.18.2 + N35/N38 + 铁律 3/4。 |

| N45 | context 编译器进写作链(M12-c 首切片) | writing.propose 增 `compileProposalContextBudgeted`(加法导出, 旧 compileProposalContext 保留兼容): 输入段按语义归层(任务 P0/焦点章结尾 P1/总纲+剧情线+篇章纲 P2/伏笔 P3)经 @novelcraft/context 的 compileContext Tier 预算淘汰(缺省 4000 tokens, 超预算先截 P4 再逐层驱逐)后以 contextSummary 产出; proposeNextChapter 与 generateNextChapter 的输入组装切换到 budgeted 版 —— 消费「context compiler core-only 无消费者」缺口(台账 §6.12)。M12-c 其余(世界书 draft→publish 闭环/memory 接线/RAG current-source typed result)续批推进。N45 review 修复(2026-09-01, P0+P1+P2 全闭): ①P0 contextSummary 只产统计行, budgeted 输入成无正文空壳(propose/generate 的 LLM 输入只剩「上下文共 N/4000 tokens」一行, 连任务指令都丢——相对旧全量拼接是静默功能回归, MockProvider 不校验输入故测试不可见)→ 正文改为渲染存活 sections(【段名】+内容 join), contextSummary 降级为尾部预算附注; ②P1 函数加 opts.budget_tokens(预算可配), 测试重写为真内容断言(【任务】段+焦点章内容进输入+附注)与小预算真驱逐用例; ③P2 llm-step 两 spec 的 inputNotes 对齐实际输入形态。伏笔归 P3(先于结构 P2 被逐)的层归取舍记录在案(连续性 vs 预算的可辩设计)。依据: 后续开发计划.md M12 + 台账 §6.12 + 铁律 4(纯加法, 旧导出保留)。 |

## 第十三批(M12-c 续批: memory 接线, 2026-09-01)

| # | 裁定 | 决定 |
|---|---|---|
| N46 | memory 事件账本接线(写链 append + dossier 投影读面) | ①dsh saveChapterGuarded 审批执行成功后 appendEvent(manual_correction 型, 幂等键 chapter_index+sequence=0, snapshot_after=保存结果摘要 content_hash/commit/skipped 不含正文; source=manual_edit)——镜像父仓「发布只快照已有事件」的最小版; append 失败容错降级不回滚正文(账本是派生真相可重建)。②store chapterDossier 增 memory 投影读面(§6.18.4 按故事顺序): chapter_index ≤ 本章过滤后 projectWorldState, 输出 events_total/events_through_chapter/broken_lines/entities_tracked/relations_tracked/last_event_at; 账本不可读时空投影降级(容错); store 新增 @novelcraft/memory 依赖(拓扑 vault→memory→store 无环)。v1 边界: 只接保存链一个写点(adopt 世界对象/深导等写点的事件 append 随对应切片推进); projectWorldState 的 created_at 排序在无并发的单用户本机下与故事顺序一致, 多源乱序写入的严格故事序重放留待 RAG/memory 深化批。N46 review 修复(2026-09-01, 3×P1+2×P2 全闭): ①P1 **写链 append 回撤** —— review 发现自动合成 manual_correction 事件直接违反台账 §6.18.4 两项裁定(manual_correction 首版 reserved 在 payload/合并语义明确前拒绝写; 普通正文发布只快照已有事件不从新正文造事件), 且 sequence=0 硬编码使每章只有首存入账、no-op 保存(skipped)可占用槽位令账本持有错误数据 —— 写点撤回, 读面保留; 事件写点待 payload 语义设计后经新裁定接入(记 M12 深化批), 派生递增 sequence(chapterCoverage.lastSequence+1)留作彼时方案。②P1 依赖门禁: dsh/package.json 补 @novelcraft/memory|outline|context(N44/N45 两处既有遗漏一并修复), writing 补 @novelcraft/context —— check:deps 回绿。③P1 排序边界升级记录: projectWorldState 的 created_at 排序在 manual_correction(v1 惰性无害)下无实害, 但写点接入前必须先改 (chapter_index,sequence,id) 排序(加法选项)。读面(§6.18.4 故事序过滤)维持。依据: 后续开发计划.md M12 + 台账 §6.18.4 + 铁律 2(账本是派生真相)。 |

| N47 | RAG current-source typed result(total/truncated/open_target) | ①core rag 加法: RagSearchResult 增 total(召回截断前数量)与 truncated(恒布尔, topK 截断标记)——调用方可信判断「8 条结果」≠「只有 8 条」(台账 §6.12.6「无 total/truncated」缺口); RagChunk 增 open_target(确定性派生不改落盘: 章正文 → chapters/NNN.md 相对路径 + start/end 偏移; outline → structure/; memory → memory/events.jsonl; 其余 → world/)——「来源重开」的最小读面。②dsh novelcraft_rag_search 回执: total/truncated 扁平字段 + hits 附 open_path/open_start/open_end; message 含 命中 n/total 与截断提示。N47 review 修复(2026-09-01, 2×P1+3×P2): ①P1 recall 顶棚不可见可造假完备信号(total=recall 触顶时用户误信全书只有 N 处)→ RagSearchResult 增 recall_capped + 工具 message 触顶标注「total 为下界非全量」; ②P1 偏移量承诺落空(chunkChapterText 不写 start/end_offset, 死管道)→ rag.ts 文档措辞降级为「前向占位, 有生产者前不承诺定位精度」; ③P2: total 措辞收紧为「topK 截断前的召回集大小(受 recall 上限约束)」; world_entity 落点粒度(world/ 而非具体对象文件)与 memory/outline 无生产者分支记为前向占位; N46 遗留的 dsh 冗余依赖声明(memory/context 无直接 import)记下次触碰时移除。依据: 后续开发计划.md M12 + 台账 §6.12.6 + 铁律 4(纯加法, 落盘格式不变)。 |

## 第十四批(clean-build 与插件边界修复, 2026-09-01)
...
| N50 | DSH `0.1.2-alpha.4` 真实 API 适配 | 以 deepseek-harness `4e84901` 与 npm alpha 标签为基线：客户端改用认证 Connection RPC，不再向 `rpc.handle` 传 method-level `authority`(安全由 DSH Host/Origin 围栏 + 签名浏览器会话承接，默认支持部署仍为 loopback)；「只读信号 + 记录决定，零 canonical 资产写」边界不变。删除对 DSH `node_modules` 的 `client/push` postinstall 补丁与自定义 remote 事件；保留动作后即时刷新、聚焦/可见性刷新和非零退避轮询。dsh/client/preset 直接 DSH peer/dev 依赖精确锁定 `0.1.2-alpha.4`，清除已退役的 client-runtime/host-apiproxy 依赖。N50 取代 N19/N49 中 method-level loopback 与 ADR-0018 自定义 push 的当前实现条款，其余 fail-closed 约束不变。 |
| N51 | DSH `0.1.2-rc.1` 兼容基线与真实插件烟测 | 以 deepseek-harness `76fda729` 和 npm `next=0.1.2-rc.1` 为基线，dsh/client/preset 的直接 DSH peer/dev 依赖继续精确锁定同一版本，不使用错误指向旧包的裸 `latest`。客户端构建 vendor 同步 rc.1 的平台模块、显式 external、CSS 和公开构建环境语义，但只保留 out-of-tree 动态 bundle 所需部分。公开 bundle 每次交付必须从干净构建产物打包，并在隔离 `DSH_HOME` 经官方 `dsh plugin` 安装，验证 profile 合成、Web boot、client factory 与 `/novelcraft/watch/state` 认证 Connection RPC；不调用真实 LLM、不写真实 Vault。N51 仅更新 N50 的版本与构建/验收基线，其安全和权限裁定保持不变。 |

## 第十六批(父仓对齐增量, 2026-09-07 用户确认)

依据: 铁律 8(父仓只读) + 台账 §6.25 持续更新协议 + `docs/agent/reviews/2026-09-07-parent-pr-alignment.md`(父仓基线 `8c1516daf`→`fede3bd72` 的只读核对)。

| # | 裁定 | 决定 |
|---|---|---|
| N52 | 父仓 PR 进展对齐判定与候选排程 | ①父仓 2026-08-31 后合并 PR 判定: #109(认证前双入口)为宿主层产品结构、#115(镜像 CVE)无本仓对应形态(audit gate 已覆盖)、#110(RP 长记忆)属 D23 延期域、#118 的主题包机制(ThemePackageV1/ZIP/IndexedDB)属父仓独立前端壳——四项**不对齐且不预建**; 此区间另有 95 个不经 PR 直推 main 的提交(含 evidence/RAG 检索加固、evidence-gated review workflows 与 RP source setup 引导等四项域级 feat), 本轮仅粗粒度归类未逐项判定, 记入台账 §6.1 并由后续相关切片核对时吸收。②接受的对齐候选(登记后续开发计划 §4b, 开工前须细化到任务级并守 §0 纪律): client 窄屏/移动批提级(#118 强化); rc.1 主题能力探测(照 A1 先例只读探测, 宿主有 seam 才做浅/深适配, 无则 truthful 不做); CI PR/main 分流评估(学 #117 模式不学数字, 先量时长); 性能基线挂真实一书验收批。③#110 的 frozen test `non_ready` 判定记入台账 §6.20 作为 D23 重开评估输入信号(非充分条件); 其评测纪律(冻结 holdout+readiness 判定+失败即停不调参)记为付费评测范式, 重开须用户费用/数据授权, 本批不排程。④台账 §6.1/§6.20/§6.24/§6.25 随本轮刷新, 后续父仓进展按 §6.25 四步协议增量更新, 不整体重审。本裁定纯方向性登记, 零代码变更。 |
| N53 | 常驻创作状态面(compaction-proof state face)经宿主 system-prompt seam 落地 | ①采官方消费者范式(dsh-user-approval 先例): 全局注册一次 `ctx.systemPrompt.section({name:'novelcraft:usage', order:300})`(静态用法说明, 稳定 prefix/KV-cache 友好)与 `ctx.systemPrompt.context({name:'novelcraft:state', order:130})`(durable user-role snapshot, compaction 不影响); context 的 text provider 按 `ac.agent.session` 分支对接 `SessionVaultBinder.peek`(新增只读同步方法), 无 agent/未绑定 → 空贡献。②重算在请求路径外(provider 必须同步, vault 验证与全量重扫均在首个 setImmediate 之后): vault 激活(0→1)预热 + `afterMutation` 漏斗(经 capabilities.read.refreshResidentState, N35 扫描合规) + 指纹漂移自愈(Git loose/packed ref OID + 每个 `.json` 信号的 name/size/mtimeNs/ctimeNs(逐路径拒绝 symlink, 不跟随 vault 外目标); 每次 assemble 同步比对, 漂移则调度重算、本次返回旧值, 一回合内收敛)。③快照范围=精简 v1(用户裁定 2026-09-08): 书名/章游标/arc-thread-scene 状态计数/逾期伏笔 top-N(判定与 radar-risk 同规则: planned_payoff<当前最大章且无 reveals_foreshadowing 边)/open 信号按 severity 计数; 不含正文、vault 绝对路径、记忆投影明细。预算 `config.prompt.maxTokens`(200–2000, 缺省 600), 降级阶梯确定性(去伏笔明细→去状态计数→仅截过长书名, 书名行/游标始终保留)。④可回放性: 静态 section 随渲染后 system 记录于 `request/header`(EpochHeader.system); 动态 context 由宿主 RuntimeContextProjection 作为 `@deepseek-ai/dsh-system-prompt` 所有的 `user/message` snapshot 持久化, 模型可见⟺可回放由宿主承接; 本面仅 logger 记 hash(sha256 前 16 hex)与 token 估算。⑤`config.prompt.enabled` 缺省 true(用户裁定); 关闭 → 零注册零 IO。⑥依赖加法: dsh peer/dev 显式钉 `@deepseek-ai/dsh-system-prompt@0.1.2-rc.1`(dsh-agent 的 peer, 运行时必在); dsh→`@novelcraft/context` 依赖恢复(resident-state 是真实消费者; N48 删的是当时无消费者的冗余, 拓扑序 context(5)<dsh(13) 合规)。Alternatives considered: ①每会话 scoped 注册 vs 全局注册+agent 分支——采后者(官方消费者同款, 免生命周期对账); ②持久 state journal 文件 vs logger hash——宿主 request/header + runtime-context `user/message` 已可回放, 自建 journal 徒增格式版本负担, 记 deferred(若未来需要跨进程对账再启); ③仅 context vs section+context 双注册——静态用法说明进稳定 prefix, 避免每次快照重复携带; ④快照含 vault 绝对路径——工具经 session 绑定解析 root, 路径非必需且扩大暴露面, 不含。已知边界: 部署设宿主 `includeRuntimeContext:false` 时快照不进模型历史(README 部署注记); vault 外部直改(不经工具/不 commit)要到下一次 afterMutation/激活/漂移检测才反映。依据: wiki《DSH 插件开发优秀范式》52/20(官方正门+KV-cache 意识)、上游 0.1.2-rc.1 `dsh-system-prompt` typings 逐字核实、用户 2026-09-08 两项裁定(精简 v1/默认开)。**二轮评审追记(2026-09-08)**: 动态快照正文统一中和 `{{`(零宽间隔)——vault 自由文本(书名/章名/伏笔名/状态键)未中和时可打断宿主 context 严格插值渲染(真实插件实证, 修复出口在 core renderResidentState); 逾期伏笔口径修正 = radar-risk 规则 + archived 排除(快照计数可与收件箱 risk 信号不一致); stopAll 后 in-flight 重算不写回缓存。**三轮评审追记(2026-09-08)**: 信号指纹拒绝 symlink 后再 stat; vault 验证移到让出请求调用栈之后; core maxTokens 最小 200, 最终降级只截过长书名并严守上界。**四轮整链复审追记(2026-09-08)**: 状态计数改用 Map 防原型键漏计; Git 指纹改读 OID, 信号指纹改为逐文件聚合; deactivate 阻止迟到重算复活缓存; afterMutation 在雷达信号落盘后才调度快照; 可回放文档区分 request/header 的静态 system 与 user/message 的动态 snapshot。 |

## 第十七批(M13-B 结构维护可复算基准, 2026-09-08 用户确认)

依据: 2026-09-08 wiki DSH 调研头脑风暴 A2 方向(优秀范式 26「可复算基准」/34「判分考题」)+ `scripts/benchmark-read-paths.mjs`/`evals/deepseek-v4-flash/` 形态先例 + 现状检测器面只读探索(六面雷达/health 六键/写链校验已核实) + 用户两项裁定。

| # | 裁定 | 决定 |
|---|---|---|
| N54 | 结构维护可复算基准(continuity benchmark)形态与边界 | ①形态: 合成种子 vault + 错误注入目录(`evals/continuity-benchmark/catalog.json` 单一事实源, scripts 消费) + core 检测器直调(零 DSH/零 LLM/零网络/只写 tmpdir) + 分类捕获率矩阵报告(`npm run bench:continuity`, console.table + 可选 `--json`)。目录三分类诚实呈现——**covered**(现状已捕获, ~15 条: failed_import/uncovered_chapter/L0 同名/alias 候选/thin_object/foreshadow_overdue/chapter_gap/显式 relations 悬空/health 六键/POV 缺失歧义/knowledge 计数)、**gap**(确定性可补, 批 2 目标 8 项: reveal_before_plant、thread_range_invalid、arc_thread_range_mismatch、thread_orphan、reveal_target_dangling、payoff_scene_dangling、legacy_edge_dangling、scene_index_conflict)、**llm-only**(POV 越界正文扫描、知识泄漏事后检测——本质需 LLM 语义判断, 只标注不实现; 付费评测仍受 N52 ③ 纪律约束)。②门禁(用户裁定 2026-09-08): 捕获率**仅报告不设 CI 硬门禁**; 退出码语义=脚本自身失败非零、MISS 不红; 未来收紧为硬门禁须再裁定。③边界: 不建日期/时间戳级 timeline 模型(台账 §6.18.6), 批 2 只做章序一致性; 新检测器只落 core 加法(assistant 新文件 `radar-continuity.ts`/outline 新导出), 归入 risk/health 既有面, 不增第七雷达面、不加工具、不动 39 工具清单与 MATRIX/preset/README seam 矩阵; 信号走既有 `Signal` 结构与 `reconcileRadarSignals` 对账契约(logical_key 稳定/observation_hash 绑载荷/裁决不复活/问题回来重开), 无新 git writer(check:git-writers 允许表不变)。④vitest 不消费 catalog(避免 dist 解析与跨包依赖门禁问题), scripts 与各包行为测试双轨, 注入面以 catalog 为单一事实源。任务级计划与批次协议见后续开发计划 §4d。Alternatives considered: ①vitest 共享 seed 工厂 vs scripts 自包含——采后者(共享工厂的依赖门禁/dist 解析成本大于漂移风险, catalog 为唯一共享物); ②硬门禁冻结基线(照 audit-baseline.json 先例)vs 仅报告——用户裁定仅报告; ③新检测并入 radar-risk.ts vs 新文件——新文件(铁律 4 只做加法, 不动既有行为路径)。依据: 用户 2026-09-08 两项裁定(范围/门禁)+ 铁律 4 + 后续开发计划 §4d + 头脑风暴记录(2026-09-08 会话)。 |
