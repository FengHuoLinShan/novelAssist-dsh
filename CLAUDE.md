...
| 辅助层 | imports / context / rag / writing | 六阶段深度导入、确定性上下文编译、检索(BM25+精排+可选 BGE 向量, L0/L1/L2)、正文审查/修订 |
| 编排核心 | llm-step / trace / assistant | 内容手原语(schema/预算/超时/journal)、trace contract 事件/断言/mock、信号/收件箱/校准 |
| DSH 接触面 | dsh(唯一) | DshProvider/ApprovalGate/domain/jobs/vault 绑定/tools 七工具 |
...
npm run build --workspaces  # 注意拓扑序: vault→trace→store→llm-step→rag→rag-bge→memory→context→outline→assistant→world→writing→imports→dsh→client
```
