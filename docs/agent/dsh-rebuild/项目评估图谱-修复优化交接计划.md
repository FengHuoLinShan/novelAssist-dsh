...
> 状态：已完成；批次 1–7、独立复审、安全修复和最终全仓门禁均已闭合。
...
| 5 | P0-T1 prepared transport integrity | 已完成 | `eed87f9b` | `approve`；关闭 tool-call block + stop 伪成功与测试 Provider finish 兼容 `289b75b8` | 完整 request admission；finish/text/outcome；disjoint usage；无 raw journal |
| 5 | P0-E1 零付费评测 dry-run | 已完成 | `2f80ab25` | `approve`；冻结精确 12 类/4 链/12 fixture，拒绝重复矩阵与缺值授权 `289b75b8` | 4×3×3=36 logical calls；`--execute` 硬拒绝；provider=0 |
| 6 | P1-R1 RAG 向量身份与降级 | 已完成 | `247a6259` | `approve`；修复 sparse array 绕过 finite 门 `96c0a565` | 非空/finite/维度/model/backend 共享校验；全失效稳定 BM25 |
| 6 | P1-C1 Outline/World 选择性 Context | 已完成 | `5223368f` | `approve`；draft 资格、actual manifest、provider/apply 漂移闭合 | 显式白名单来源；draft/candidate 默认拒绝；Outline apply 前重验 |
| 6 | P1-M1 Memory/POV 最小竖切 | 已完成 | `e278edb8` | `approve`；修复缺失/多 POV fail-open `96c0a565` | 截止章已知/exclusion 脱敏 P3；生成/独立审查/采用重验 |
| 7 | P1-U1 长任务查看与恢复 | 已实现 | `079c0dbc`；复审修复 `86034c2d` | `approve`；关闭 run/checkpoint symlink 与 browser root 泄漏 | 四态作者卡；inspect/resume/start-new/abandon 回助手受控执行；390px 基础样式 |
| 7 | P1-U2 书库切换 | 已实现 | `e5684279`；复审修复 `86034c2d` | `approve`；关闭同标题切书、坏 Vault 与本地状态残留 | 未绑定可列书；create/open 回助手；切书先清旧读面并拒绝晚到响应 |
| 7 | P1-U3 总纲与 Scene 工作区 | 已实现 | `4b76cf4d`；复审修复 `86034c2d` | `approve`；关闭外部 proposal apply 与扫描根 symlink | outline preview 作者投影；显式来源/任务/目标；apply 不经 client 写 canonical |
| 7 | P1-U4 World Bible 工作区 | 已实现 | `2915a486`；复审修复 `86034c2d` | `approve`；关闭对象/Bible 根 symlink 与内部错误泄漏 | 五模式、来源选择、对象/页面作者投影；draft 发布回 ApprovalGate 路径 |
| 7 | P2-X1 整书备份与恢复 | 已实现 | `7f7425f6`；复审修复 `86034c2d`、`1abdc9d5` | `approve`；关闭凭据/sidecar/目标路径与伪 HEAD/缺骨架 | 初始化验证 + manifest + SHA-256；源漂移/目标冲突/symlink/疑似凭据 fail-closed |
| 7 | P2-X2 Word 发布包 | 已实现 | `2e800805`（锁文件 `a0533e3e`）；复审修复 `86034c2d` | `approve`；关闭外部读取、源内目标与未进 HEAD 的资产 | clean current chapters → OOXML `.docx` + 合法图片 + receipt；失败零半成品 |
| 7 | P2-A1 外部 AI 技术雷达 | 已实现 | `5c4bdcf8` | `approve`；工作簿单元格与官方链接复核 | `05_AI平台` 官方来源刷新；四级证据分列；零付费调用 |
...
批次 5 最终门禁：四项 `check:*`、`npm run build`、`npm run typecheck`、`git diff --check`、
`npm test`（16/16 workspace；LLM-step 159；DSH 264；Imports 180；Store 385 + 19 + 26；
World 158；Writing 97）全部通过。独立复审关闭 1 个 Blocker 和 2 个 Major：实际含 tool-call
block 但 finish=stop 时 llm-step 不得覆盖 provider 的严格 outcome、重复 fixture 不得伪装成
4×3×3 完整矩阵、`--authorization` 缺值不得静默降级。`eval:pilot:self-test` 固定
provider 调用为 0、logical calls 为 36；真实付费执行器尚不存在，`--execute` 明确失败。
本批未调用付费模型，未改父仓库，未触碰既存未跟踪测试副本。

批次 6 最终门禁：四项 `check:*`、`npm run build`、`npm run typecheck`、`git diff --check`、
`npm test`（16/16 workspace；Context 13；Memory 10；RAG 53；Outline 27；World 159；
Writing 104；DSH 268；Imports 180；Store 385 + 19 + 26）全部通过。独立复审先发现
2 个 Major：稀疏 JS 数组可绕过 `Array.every` finite 校验，以及缺失/多 POV 时
sanctioned 生成仍可 fail-open；两项均由 `96c0a565` 关闭后复审 `approve`。World
页面建议的 freshness 合同明确只到生成时；其后 draft 可由作者编辑，generic adopt
审批当下 draft 字节，本批不扩张为 selected-specific publish 合同。本批未调用付费模型，
未改父仓库，未触碰既存未跟踪测试副本。

批次 7 已完成实现、独立复审与最终全仓门禁：U1–U4 均复用现有
client/RPC/助手受控写面，没有给 loopback RPC 增加 canonical 写权；X1/X2 均为 stdlib 为主的
显式 CLI，前后快照门检测源漂移，自检覆盖目标冲突、symlink/凭据、输入失败和零半成品；A1 只人工刷新现有 Excel，不新增爬虫、
数据库或定时任务。X2 的 bundled LibreOffice 字体集不能完整显示 CJK，因此只把它当结构渲染门；
最终视觉核验使用本机 LibreOffice 与已安装 CJK 字体，3 页均可读。批次 7 未调用付费模型、未改
父仓库、未触碰既存未跟踪测试副本，也未 push。最终门禁为：四项 `check:*`、拓扑
`npm run build`、`npm test`（16/16 workspace；DSH 271；Client 65；Imports 180；
Store 386 + 19 + 26；World 162；Writing 104）、全仓 `npm run typecheck`、
`eval:pilot:self-test`（36 logical calls / provider=0）、两项 Vault 自检和 `git diff --check` 全绿。
...
> 下表固定 `d4d8f4858e103055779057b7d8e23adbb09bf42d` 的诊断事实，用于解释任务来源；
> 它不是当前 HEAD 状态。每项是否关闭只以上文“实施进度”和对应提交/门禁为准。
...
**实施结果（`079c0dbc`）：** 新增 workflow 作者卡和只读 RPC 投影，四类状态与可执行动作均用作者语言显示；按钮只向助手发送确定性请求，client 不直接改变 run 或 canonical。复用 `useWatch` 刷新，补空态/失败态文案和 focus-visible/390px 基础 CSS；尚无 React/真实宿主 390px E2E。没有新增任务数据库或后台队列。

#### P1-U2：书库切换（S）

复用 book list/create/open 能力，做单一书库入口；不与 outline/world 页面混成一个提交。会话与 vault root 切换后先清旧读面，拒绝旧 session 晚到响应。

**实施结果（`e5684279`）：** `books/list` 在未绑定会话也可返回 `{book,title,current}`，不暴露 root；新建/打开仍由助手调用既有受控能力。共享 `useSessionView` 在 session/root 变化时先清空旧数据，并忽略旧请求晚到结果；未实现删除或回收站。
...
**实施结果（`4b76cf4d`）：** 复用现有 Story Map，增加总纲预览列表、显式来源选择、任务/目标和可选工作稿开关；preview/apply 都转交助手执行。核心只加法导出 `listOutlinePreviews()`：坏记录和单文件 symlink 不进入列表，不安全的 proposals 根 fail-closed；章节仍是主导航，未扩建 Scene CRUD/时间线/三审计。
...
**实施结果（`2915a486`）：** 单一 `WorldBibleAction` 覆盖五种既有生成模式、显式来源、可选工作稿和新建页；`world/workspace` 不返回正文、provenance 或 raw JSON，保留有界 `source_ref` 仅供复选和受控助手请求，UI 不直接显示该内部引用。建议落 draft，发布通过助手调用既有 `novelcraft_store_adopt` 与 ApprovalGate；同名页修订、影响预演和历史恢复仍未扩建。
...
**实施结果（`7f7425f6`，复审修复 `86034c2d`、`1abdc9d5`）：** `scripts/vault-portability.mjs` 提供 `backup`、`restore`、`verify` 与 `--self-test`。三种路径均复用 `validateInitializedVault`，只接受真实可解析 HEAD、合法 book 和完整骨架；复制闭包含 `.git`、ignored Atlas 图片和 durable workflow 状态。前后重算 mode/size/SHA-256 manifest，源漂移、symlink、疑似凭据、非 Vault、已有/悬空目标均失败；文件名凭据、Git config 内嵌认证及外部 sidecar symlink 也会拒绝。先写同级临时目录再发布，失败不删除源、不覆盖目标；不实现 destructive move、压缩包或云备份。

#### P2-X2：Word/发布导出

迁移恢复与创作事实稳定后单独定义导出合同、版本来源、资源打包和失败回执；不与 X1 共用完成状态。

**实施结果（`2e800805`，复审修复 `86034c2d`）：** `scripts/vault-publish.mjs export` 只接受 HEAD 已跟踪、当前字节与 HEAD blob 一致、严格编号且正文 hash 匹配的 current chapters，生成真实 OOXML `manuscript.docx`、`world/atlas/images` 内全部 magic 合法 PNG/JPEG 资源和 source receipt。安全目标解析成功后的构建失败会留下结构化失败回执；源/目标 preflight 失败只非零退出且零产物。两类失败都不发布半成品。`--self-test`、ZIP 结构、可访问性审计及 3 页 LibreOffice 视觉核验通过。它不是 adopted-page-only 资源精选，也不是 comments/track-changes round-trip，不回写 Word。
...
**实施结果（`5c4bdcf8`）：** 只更新事实附件的 `05_AI平台`，保留原有 Sheet 结构与样式。DeepSeek、OpenAI、Anthropic、Google、Alibaba 八个代表模型均记录核对日和官方 URL；官方规格、官方 benchmark、第三方 benchmark、项目实测分列。项目实测继续明确为“未运行付费小说评测”，不从厂商通用 benchmark 推导小说建议质量。
...
flowchart TD
  ...
  T1 --> E1[P0-E1 zero-cost eval dry-run]
  I1 --> E1
  W1 --> E1
...
上述依赖顺序已经执行完毕。E1 当前只完成零付费 dry-run 与授权硬门；真实小说质量 pilot 仍是独立、未授权任务，不因批次 7 或厂商规格刷新而自动启动。
...
npm test
npm run typecheck
npm run vault:portability:self-test
npm run vault:publish:self-test
git diff --check
```
...
4. 相关台账状态和本文任务状态同步；
5. 全仓门通过；
6. commit 只含本任务文件；A1 可单独提交其事实附件，其他任务不得捎带 `outputs/` 或无关 untracked 文件。
...
本计划中的代码工作包已经落地。下一位 Agent 应先核对 §0 的最终复审与门禁记录，再按以下边界工作：

- 没有用户对费用、fixture 和数据外发的明确授权，不运行真实小说质量 pilot；
- AI 平台规格只人工刷新现有 Excel，不另建维护系统；
- 带批注/修订跟踪的 Word round-trip、回收站、云备份、Scene CRUD/三审计仍是未实施范围，不能从本批的 `.docx` 发布包或辅助工作台外推；
- 继续保持 client loopback 只读/记录决定，所有 canonical 写入回助手、领域命令与 ApprovalGate。
...
同一独立子代理按代码可达性而非文档声明，复核 `079c0dbc..5c4bdcf8`、当前工作簿和三份交接文档；
并用临时 Vault 实际构造越界、symlink、ignored/untracked 和同标题切书负例。初次结论为
“存在阻塞，修复前不可通过”，发现的问题由 `86034c2d` 与 `1abdc9d5` 全部关闭：

| 级别 | 实证问题 | 修复与回归 |
|---|---|---|
| Blocker | outline proposal 文件 symlink 可在 ApprovalGate 后读取 Vault 外记录并写 canonical | proposal 根/文件逐次 containment + lstat；外部记录即使带完整 context receipt 也零 canonical 写 |
| Blocker/Major | 同标题不同 root 不触发切书事件；U3/U4 本地任务/选源跨书残留 | 以 `bound.root` 判书身份；同会话事件清空 task/source/draft/chapter/notice/open；旧响应代际失效 |
| Major | workflow run/checkpoint、book、Bible/object、Store 扫描根可跟随 symlink 读出 Vault 外数据 | 在共享读 seam 每次 guard/lstat；书 list/create/open 只接受完整初始化 Vault；RPC 错误不回传外部路径 |
| Major | X1 可复制 Git config 内嵌 token，或覆盖 dangling sidecar；verify 可跟随外部 sidecar | 扩充凭据门；目标/sidecar 用 lstat 识别悬空条目；verify 只读普通 sidecar；负例保留原链接 |
| Major | X1 只看 `book.yml + .git/`，伪 HEAD、缺骨架目录也会被 backup/restore/verify 判成功 | `1abdc9d5` 复用 `validateInitializedVault`；self-test 改用 `initVault`，并固定 fake HEAD/缺 bible 拒绝与恢复后再验证 |
| Major | X2 可把目标写回源 Vault、读取外部 chapters/images，或发布从未进入 HEAD 的 ignored 资产 | canonical source/target；路径链 symlink 拒绝；图片扩展名 + magic；book/每章 HEAD blob 与当前字节逐项相等 |
| Minor | 新 workflow wire 暴露未使用绝对 root，文档夸大自检、内部来源和 390px 证据 | workflow bound 收窄为 book；文档限定 `books/list`、source_ref 用途、preflight/receipt 与 CSS/E2E 边界 |

修复后独立复跑：DSH 29/29、Client 65/65、Outline 29/29、World 定向测试、相关 typecheck、
两项 CLI self-test 与 `git diff --check` 全绿，未发现新 Blocker/Major，代码 verdict 为 `approve`。
其后 §0 所列最终全仓门禁全部通过；本节与 §0 共同作为交接证据。
...
- 作者工作台：`packages/novelcraft/client/src/client/{WorkflowAction,BookLibraryAction,StoryMapAction,WorldBibleAction}.tsx`
- UI 只读投影：`packages/novelcraft/client/src/{rpc,wire}.ts`、`packages/novelcraft/dsh/src/client-face.ts`
- 整书可移植性：`scripts/vault-portability.mjs`
- Word 发布包：`scripts/vault-publish.mjs`
- AI 技术雷达：`outputs/01a05ade-6b8d-7f02-bfaa-2d4e3f2df08c/novelAssist-dsh-项目评估图谱-2026-09-01.xlsx`
